...
class AboutScope < Neo::Koan
  ...
  def test_bare_bones_class_names_assume_the_current_scope #bare bones class names assume the current scope
    assert_equal true, AboutScope::String == String
  end

  def test_nested_string_is_not_the_same_as_the_system_string   #nested string is not the same as the system string
    assert_equal false, String == "HI".class
  end

  def test_use_the_prefix_scope_operator_to_force_the_global_scope    #use the prefix scope operator to force the global scope
  ...
  def test_constants_can_be_looked_up_explicitly    #constants can be looked up explicitly
    assert_equal true, PI == AboutScope.const_get("PI")
    assert_equal true, MyString == AboutScope.const_get("MyString")
  end

  def test_you_can_get_a_list_of_constants_for_any_class_or_module    #you can get a list of constants for any class or module
